...
export interface Fast404RouteMatcher {
  ready: boolean;
...
export function normalizeFast404RequestPath(requestPath: string, baseURL: string): string {
  return withoutBase(withoutQuery(requestPath), baseURL);
}

export function createFast404RouteMatcher(
