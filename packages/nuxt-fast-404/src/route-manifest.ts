import type { Fast404Manifest, Fast404PageRoute, Fast404RouteSource } from './types/module';
...
function getPageAliases(page: Fast404PageRoute): string[] {
...
export function collectNuxtPageRoutePatterns(
  pages: readonly Fast404PageRoute[],
  parentPath = '',
): string[] {
  const routes: string[] = [];
...
export function collectServerRoutePatterns(sources: readonly Fast404RouteSource[]): string[] {
  return [
    ...new Set(
...
export function createFast404ManifestSource(manifest: Fast404Manifest): string {
  return [
