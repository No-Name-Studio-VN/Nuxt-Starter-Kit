...
import { createFast404RouteMatcher, normalizeFast404RequestPath } from '../src/runtime/matcher';

describe('fast 404 route matcher', () => {
  const matcher = createFast404RouteMatcher(
  ...
  it('normalizes base URLs and request queries before matching', () => {
    expect(normalizeFast404RequestPath('/studio/Reader/42?chapter=2', '/studio/')).toBe(
      '/Reader/42',
    );
  });

  it('fails open if Nuxt did not expose any page routes', () => {
    expect(createFast404RouteMatcher([], ['/api/story/:id'], false, []).ready).toBe(false);
  });
});
