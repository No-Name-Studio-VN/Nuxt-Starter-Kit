...
import { createFast404Response } from '../src/runtime/response';
...
describe('fast 404 responses', () => {
  it('returns a small cacheable HTML response for browser navigation', async () => {
    const response = createFast404Response('/missing', 'GET', 'text/html', options);
  ...
  it('keeps unknown API responses JSON even when HTML is accepted', async () => {
    const response = createFast404Response('/api/missing', 'POST', 'text/html', options);
  ...
  it('does not send a body for HEAD requests', async () => {
    const response = createFast404Response('/missing', 'HEAD', 'text/html', options);

    expect(await response.text()).toBe('');
  });

  it('disables caching when cacheMaxAge is zero', () => {
    const response = createFast404Response('/missing', 'GET', 'text/html', {
      apiPrefixes: ['/api'],
      cacheMaxAge: 0,
    });
