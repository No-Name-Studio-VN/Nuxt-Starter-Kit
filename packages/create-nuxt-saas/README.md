...
- `status` — installed modules, files you have modified, damaged markers, and whether an update exists.
  Reads only local state, so it is instant and works offline. When the registry follows a branch it
  reports the branch rather than guessing, because whether that branch moved is a network question.
...
Generated projects carry `.nuxt-saas/manifest.json`, recording the modules installed, the kit revision
they came from, and a hash per file. Do not delete it — upgrades depend on it.

The registry follows a branch (`kit.revision: "main"`), so anyone can point `kit.template` at their own
fork and channel. A project cannot follow a branch, though: upgrading re-renders the revision the
project came from and three-way merges it against the new one, so the manifest has to keep naming the
same tree forever. The channel is therefore resolved to a commit when a project is generated or
upgraded, and that commit is what gets written down.
