...
import type { OAuthLoginCompletionResult, OAuthProfile } from '~~/types/auth';
import { apiRoutes } from '#shared/apiRoutes';
import userService from '~~/server/utils/database/user';
import oauthAccountService from '~~/server/utils/database/oauthAccount';

export type { OAuthLoginCompletionResult, OAuthProfile } from '~~/types/auth';
    ...
    if (existingLink) {
      if (existingLink.userId === currentUserId) {
        return sendOAuthRedirect(
          event,
          `/settings/security?error=oauth-already-connected&provider=${provider}`,
        );
      }
      return sendOAuthRedirect(
        event,
        `/settings/security?error=oauth-already-linked&provider=${provider}`,
      );
    }

    // Check if user already has a different account for this provider linked
    const userProviderLink = await oauthAccountService.getByUserAndProvider(
      currentUserId,
      provider,
    );
    if (userProviderLink) {
...
): Promise<OAuthLoginCompletionResult> {
