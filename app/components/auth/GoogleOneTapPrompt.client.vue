...
import type { GoogleCredentialResponse } from '~~/types/googleOneTap';
import { cancelGoogleOneTapPrompt, loadGoogleIdentityScript } from '@/utils/googleOneTap';
...
async function submitCredential(response: GoogleCredentialResponse) {
  try {
    const result = await apiRequest<ApiResponse<GoogleOneTapPayload>>(
      apiRoutes.AUTH_GOOGLE_ONE_TAP,
      {
        method: 'POST',
        body: {
