<template>
  <SidebarMenu>
    <SidebarMenuItem>
      <SidebarMenuButton
        :class="
          cn('data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground')
        "
  ...
  >
    <DialogHeader>
      ...
      <DialogDescription> Get the best experience with our progressive web app </DialogDescription>
    </DialogHeader>

    <div class="space-y-4">
      <ItemGroup class="space-y-3">
        <template v-for="i in INSTALL_PROMPTER_FEATURES" :key="i.title">
          <Item variant="outline">
            <ItemMedia variant="icon" :class="`bg-${i.colorClass}-500/10`">
              <component :is="i.icon" :class="`text-${i.colorClass}-600`" />
            </ItemMedia>
            <ItemContent>
    ...
    <DialogFooter class="flex-col sm:flex-row gap-2">
      <Button variant="ghost" class="w-full sm:w-auto" @click="isDialogOpen = false">
        Not Now
      </Button>
      <Button class="w-full sm:w-auto group" :disabled="!canInstall" @click="onInstallPWA">
        Install App
