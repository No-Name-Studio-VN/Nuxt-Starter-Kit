<template>
    ...
    >
      ...
      >
        <NuxtLinkLocale
          ...
          :to="link.to"
          :target="link.target"
          class="flex w-full gap-1 underline-offset-4 hover:underline not-first:pt-3"
        >
          <CtIcon v-if="link.icon" :name="link.icon" class="mr-1 self-center" />
          {{ $t(link.title) }}
          <ArrowUpRightIcon class="text-muted-foreground ml-auto self-center" :size="13" />
        </NuxtLinkLocale>
      </div>
      <div v-if="iconLinks" class="text-muted-foreground">
        <NuxtLinkLocale v-for="(link, i) in iconLinks" :key="i" :to="link.to" :target="link.target">
          <Button size="icon" variant="ghost" class="size-7">
            <CtIcon v-if="link.icon" :name="link.icon" />
          </Button>
        </NuxtLinkLocale>
      </div>
      <div class="grow" />
  ...
  <Collapsible v-else v-model:open="isOpen" class="block w-full text-sm lg:hidden">
...
const toc = ref<{ links: TocLink[] }>({ links: [] });
