...
const destructiveSet = computed(() => new Set(props.destructiveItems));
const isDestructive = (item: string) => destructiveSet.value.has(item);
</script>
  ...
  >
    ...
    >
      <div
        :class="
          cn(
            'flex h-full items-center gap-3 rounded-xl border px-4 transition-[border-color,background-color,box-shadow] duration-200',
            item === activeItem
              ? 'border-foreground/15 bg-card shadow-xl shadow-black/20'
              : isDestructive(item)
