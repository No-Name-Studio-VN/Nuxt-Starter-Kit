<template>
  <UseTemplate>
    <Card
      ...
      :class="[to && 'hover:bg-muted', inStack && 'mb-0 rounded-none border-none shadow-none']"
    >
      ...
      >
        <CtIcon :name="icon" :size="iconSize" :class="cn(!horizontal && 'mb-2')" />
        <div class="flex flex-col gap-1.5">
          <CardTitle v-if="title || $slots.title" class="text-2xl tracking-tight">
            <slot name="title" mdc-unwrap="p" />
            {{ title }}
          </CardTitle>
          <CardDescription v-if="description || $slots.description" class="text-sm">
            <slot name="description" mdc-unwrap="p" />
            {{ description }}
          </CardDescription>
        </div>
      </CardHeader>
      <CardContent v-if="content || $slots.content || $slots.default" class="p-6 pt-0">
        <slot name="content" mdc-unwrap="p" />
        <slot mdc-unwrap="p" />
      </CardContent>
      <CardFooter v-if="footer || $slots.footer" class="p-6 pt-0">
