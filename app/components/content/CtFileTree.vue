<template>
  <Card
    class="relative overflow-hidden py-0 gap-0 rounded-lg bg-card shadow-xs not-first:mt-5 not-last:mb-5"
  >
    ...
    <div class="bg-muted/30 w-auto p-2">
      <CtFileTreeRoot :tree="parsedTree" :show-arrow :show-icon :level="0" />
    </div>
  </Card>
</template>
...
function getTree(tree: InputTreeItem[]) {
  ...
  for (const item of tree) {
    if (typeof item === "string") {
      ...
      for (const key of Object.keys(item)) res.push(getItem(key, "folder", item[key]));
    }
  }
