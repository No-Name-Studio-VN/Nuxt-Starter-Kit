...
const { data: posts, status } = await useAsyncData('blog-posts', () =>
  queryCollection('content').where('path', 'LIKE', '/blogs/%').order('publishedAt', 'DESC').all(),
);
...
<template>
  <div class="container mx-auto px-4 py-16 lg:py-24 max-w-6xl font-sans">
    ...
    <template v-else>
      ...
      >
        ...
        >
          <div class="relative w-full aspect-4/3 rounded-3xl overflow-hidden mb-6 bg-muted">
            ...
            <div
              v-else
              class="size-full bg-primary/5 flex items-center justify-center transition-transform duration-300 group-hover:scale-105"
            >
              <Newspaper class="size-12 text-primary/20" />
            </div>
          </div>
          ...
          <h2
            class="text-xl/snug font-bold text-foreground mb-3 group-hover:text-primary transition-colors line-clamp-2"
          >
            {{ post.title }}
          </h2>
          <p class="text-muted-foreground line-clamp-2 leading-relaxed grow">
            {{ post.description }}
          </p>
        </NuxtLink>
      </div>
      ...
      >
        <SearchXIcon class="size-12 text-muted-foreground/50 mb-4" />
    ...
    <hr v-if="filteredPosts.length > 0" class="border-border mb-8" />
  </div>
</template>
