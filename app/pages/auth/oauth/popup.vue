...
import { parseOAuthPopupCompleteMessage } from '@/utils/oauthPopupComplete';
...
const oauthPopupCompleteStorage = useLocalStorage<OAuthPopupCompleteMessage | null>(
  ...
  null,
  {
    ...
    writeDefaults: false,
    serializer: {
      read: parseOAuthPopupCompleteMessage,
...
function notifyOAuthComplete(payload: OAuthPopupCompleteMessage) {
  ...
  try {
    oauthPopupCompleteStorage.value = payload;
    oauthPopupCompleteStorage.value = null;
  } catch {
...
<template>
  <div class="flex min-h-svh items-center justify-center bg-background px-6 text-center">
    <div class="space-y-2">
      <h1 class="text-lg font-semibold">Completing sign in…</h1>
